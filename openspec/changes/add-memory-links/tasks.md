...
- [ ] 1.1 Add `memory_links` to `SCHEMA_SQL` in `src/storage/sqlite.ts` (after the
...
- [ ] 1.2 Add `MemoryLinkRelation` (the five-value union) and `MemoryLinkRecord`
...
- [ ] 1.3 Extend `SearchResult` (`src/domain/types.ts:84-108`) with optional
...
- [ ] 2.1 Add to the `SqliteStorage` interface (`src/storage/sqlite.ts:70`, near
...
- [ ] 2.2 Implement `addMemoryLink`: `SELECT` first for an existing `(from_id, to_id,
...
- [ ] 2.3 Implement `listMemoryLinks`: `SELECT * FROM memory_links WHERE from_id = ? OR
...
- [ ] 2.4 Implement `removeMemoryLink`: `DELETE FROM memory_links WHERE from_id = ? AND
...
- [ ] 2.5 In `deleteMemory` (`src/storage/sqlite.ts:615-623`), add `DELETE FROM
...
- [ ] 3.1 Add `RelateInputSchema` to `src/domain/schemas.ts` (near
...
- [ ] 3.2 Add `follow_links: z.boolean().optional().default(false)` to
  `RecallInputSchema` (`src/domain/schemas.ts:38-46`).
- [ ] 3.3 Add the `relate` tool definition to `MCP_TOOL_DEFINITIONS` in
...
- [ ] 3.4 Add `follow_links` to the `recall` tool's `inputSchema.properties` in
...
- [ ] 4.1 Add `handleRelate` to `src/tools/index.ts` (near `handleReview`,
...
- [ ] 4.2 `action: 'add'`: fetch both memories via `getMemoryById` (`notFound` if
  ...
  created }`.
- [ ] 4.3 `action: 'remove'`: call `removeMemoryLink`; `notFound` if it returns `false`;
  otherwise return `{ removed: true, from_id, to_id, relation }`.
- [ ] 4.4 `action: 'list'`: fetch the memory via `getMemoryById` (`notFound` if
...
- [ ] 4.5 In `handleRecall` (`src/tools/index.ts:156-205`), after the existing
...
- [ ] 5.1 `addMemoryLink`/`listMemoryLinks`/`removeMemoryLink` unit tests in
  ...
  edge.
- [ ] 5.2 `deleteMemory` cascade test: creating a link then deleting either endpoint
  removes the `memory_links` row (query directly or via `relate list` on the surviving
  endpoint returning empty).
- [ ] 5.3 `relate` tool tests in `src/tools/index.test.ts` (or a co-located
  ...
  `relation` filters.
- [ ] 5.4 `recall` `follow_links` tests: default `false` leaves `recall` output
...
- [ ] 5.5 Regression: `review`'s `archive` action on a memory with existing links
...
- [ ] 6.1 Update `CLAUDE.md`'s canonical tool list (`CLAUDE.md`, "MCP surface"
  section) to add `relate` to the `Registered:` line.
- [ ] 6.2 Add a `### \`relate\`` section to `README.md`'s "MCP Tools Reference" (near
...
- [ ] 6.3 Bump `package.json` `version` (currently `1.11.0`).
- [ ] 6.4 Run `npm run lint` and `npm test`; both must pass before this change is
  considered done.
