...
- [x] 1.1 Add `pinned: boolean` to `MemoryRecord` (`src/domain/types.ts:37-69`).
- [x] 1.2 Add the column and index to the SQLite schema
...
- [x] 1.8 Add `pinned` to the `Pick<...>` type and the returned object in
...
- [x] 1.9 Add `listPinnedMemories(namespace: string): MemoryRecordWithoutEmbedding[]`
...
- [x] 3.4 Implement explicit-set-wins merge semantics for `pinned` on both
  ...
  `:432-...`).
- [x] 3.5 Enforce `defaults.pin_limit_per_namespace` in `handleRemember`
...
- [x] 4.1 In `buildInjectPayload` (`src/resources/index.ts:205-308`), add a
  ...
  `memoriesCount`.
- [x] 4.2 Exclude pinned memory IDs from the recency (`listMemories`) and
...
- [x] 4.3 Confirm (and comment, where the code doesn't already make it
...
- [x] 4.4 Confirm the pinned step runs identically for `memory://inject` and
  `memory://inject/{hint}` (i.e., before the `trimmedHint` branch at
  `src/resources/index.ts:274-281`, not duplicated inside it).

## 5. Tests

- [x] 5.1 `remember` sets `pinned` on ADD (explicit `true`/`false`/omitted →
  default `false`).
- [x] 5.2 `remember` UPDATE merge: omitting `pinned` preserves the existing
  memory's pin state; passing `pinned` explicitly overrides it (both
  directions: pin → unpin, unpin → pin).
- [x] 5.3 `tag` sets/unsets `pinned` without touching `content`/`tags`/
  `updated_at`-adjacent fields beyond what tagging already touches.
- [x] 5.4 Pin cap: the `(pin_limit_per_namespace + 1)`th pin via `remember`
...
- [x] 5.9 `auto_inject.pinned_enabled: false` restores byte-for-byte the
  pre-this-change payload for a namespace with pinned memories (i.e., pins
  are stored but inert for inject purposes).
- [x] 5.10 Budget: pinned content that alone exceeds the memory section's
  ...
  does today.
- [x] 5.11 Durability: `pinned: true` round-trips through `toQdrantPayload` /
...
- [x] 5.12 Regression: `search`/`recall` result ordering and `SearchResult`
  shape are unaffected by a memory's `pinned` state (no new field, no score
  change).

## 6. Docs

- [x] 6.1 Add a `pinned` row to the Memory Data Model table in `README.md`
  (`README.md:899-924`) and the four translations.
- [x] 6.2 Document `pinned` in the `remember` and `tag` tool reference tables
  (`README.md:2300-2310`, `:2452-2456`) and the four translations.
- [x] 6.3 Document the pinned-memory step, its budget source, the
...
- [x] 6.4 Bump `package.json` version.
- [x] 6.5 `npm run lint` and `npm test` pass.
