...
- [ ] 1.1 Add `consolidation` to the Zod config schema (`src/config/index.ts`, near the
...
- [ ] 1.2 No new env vars — `.env.example` and `AGENTS.md`'s "Config vs. environment"
...
- [ ] 2.1 Extend `LifecycleAuditDetails.action` (`src/domain/types.ts:200`) with
...
- [ ] 3.1 Add `QdrantStore.findNeighborsById(namespace, collection, pointId, topK,
...
- [ ] 3.2 Route through `executeWithBreaker` like every other `QdrantStore` method that
  calls the client.

## 4. The consolidate tool

- [ ] 4.1 Schema: `ConsolidateInputSchema` in `src/domain/schemas.ts` (model on
...
- [ ] 4.2 Tool metadata entry in `src/tools/schemas.ts` (append after `repair`, mirror
  the `review` entry's JSON-Schema shape at lines 170-186).
- [ ] 4.3 Handler `handleConsolidate` in `src/tools/index.ts`, wired into `dispatch`'s
  switch (line 114-131) as `case 'consolidate': return handleConsolidate(...)`.
- [ ] 4.4 `list`: page through `sqlite.listMemories(namespace, config.consolidation
...
- [ ] 4.5 `merge`: load `target_id` and every `source_ids` entry via
...
- [ ] 4.6 `ctx.metrics.setGauge('bhgbrain_memory_count', ...)` update after `merge`,
  matching the pattern `review`'s `archive`/`restore` actions already follow
  (`src/tools/index.ts:390`, `453`).

## 5. Tests

- [ ] 5.1 `findNeighborsById`: returns neighbors ≥ threshold excluding self; empty
  array for a never-written collection (no thrown error).
- [ ] 5.2 `list`: clusters memories connected by a ≥-threshold edge within one scanned
...
- [ ] 5.3 `merge`: happy path — tags unioned, importance maxed, `merged_from` set (both
...
- [ ] 5.4 `merge` rejects: `target_id` inside `source_ids`; mixed-collection input;
...
- [ ] 5.5 `merge` partial failure: simulate a mid-loop `deleteMemory` throw (mock) and
  assert `merged`/`failed` both populate correctly and no source is left both archived
  and un-deleted (or vice versa).
- [ ] 5.6 Config: `consolidation.enabled: false` — decide and test the handler's
...
- [ ] 6.1 Add `consolidate` to `CLAUDE.md`'s canonical MCP tools list (currently:
...
- [ ] 6.2 Add a `### consolidate` section to `README.md`'s § MCP Tools Reference
...
- [ ] 7.1 `npm run lint` (tsc --noEmit + eslint, no `any` casts).
- [ ] 7.2 `npm test` — full suite green, including the new tests from section 5.
