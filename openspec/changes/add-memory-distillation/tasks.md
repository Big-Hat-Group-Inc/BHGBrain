...
- [x] 1.1 Add `derived_from: string[] | null` to `MemoryRecord`
...
- [x] 1.2 Add `'DISTILL'` to `LifecycleAuditOperation` (`src/domain/types.ts:175`)
  and `'distill'` to `LifecycleAuditDetails['action']` (`src/domain/types.ts:200`).
- [x] 1.3 Add `'distillation'` to `MemorySource` (`src/domain/types.ts:5`) and
  `MemorySourceSchema` (`src/domain/schemas.ts:8`).
- [x] 1.4 Add a `derived_from TEXT` column to the migration list in
...
- [x] 1.5 Thread `derived_from` through `rowToMemory` (`src/storage/sqlite.ts:1659-
...
- [x] 1.6 Add `retention.distillation` to the Zod config schema
...
- [x] 1.7 Confirm no new env var is needed: `retention.distillation` reuses
...
- [x] 2.1 Extend `QdrantAdapter.scrollAll` (`src/storage/qdrant.ts:361-386`) with an
...
- [x] 2.2 Add `src/pipeline/distillation-cluster.ts`: given a namespace+collection's
...
- [x] 2.3 Unit tests for `distillation-cluster.ts`: disjoint clusters, a
...
- [x] 3.1 Add `src/pipeline/distillation-llm.ts`: a `DistillationLLMClient` mirroring
...
- [x] 3.2 Define the prompt contract: input = cluster's memory contents (oldest to
...
- [x] 3.3 Unit tests for `distillation-llm.ts` with a mocked `fetch`: success path,
...
- [x] 4.1 Add `src/pipeline/distillation.ts` (or `src/backup/distillation.ts` —
...
- [x] 4.2 `runOnce(options?: { dryRun?: boolean })`: for each eligible
...
- [x] 4.3 On a confirmed write, archive the cluster's source memories through the
...
- [x] 4.4 Failure handling: if archival/deletion of sources fails after a
  ...
  T1 memory).
- [x] 4.5 Return a `DistillationResult` (mirroring `GarbageCollectionResult`,
  ...
  summaries).
- [x] 4.6 Integration tests: end-to-end cluster → distill → archive happy path;
...
- [x] 5.1 Add a `DistillationScheduler` in `src/backup/scheduler.ts` (or a sibling
...
- [x] 5.2 Wire it in `src/index.ts` alongside the existing
...
- [x] 5.3 Add a `bhgbrain distill` CLI command in `src/cli/index.ts`, mirroring the
  `gc` command (lines 229-236): `--dry-run` flag, prints cluster candidates and
  (when not dry-run) distillation/skip/archive counts.
- [x] 5.4 CLI test coverage mirroring `src/cli/index.test.ts`'s existing `gc`
...
- [x] 6.1 Add `bhgbrain_distill_duration_ms` (histogram),
  ...
  186`.
- [x] 6.2 Add a `distillation` field to the retention health snapshot
...
- [x] 7.1 `MemoryLifecycleService`/`assignTier` regression: `explicitTier: 'T1'`
  with `source: 'distillation'` still resolves to `T1` (confirms no new branch is
  needed per design.md Decision #6).
- [x] 7.2 `rowToMemory`/insert round-trip test for `derived_from`: write, read back,
  `null` for ordinary writes, populated array for distillation writes.
- [x] 7.3 `search`/`recall` regression: a distilled memory (`source:
  'distillation'`) ranks, filters, and displays like any other T1 semantic memory —
  no special-casing leaks into the read path.

## 8. Validation

- [x] 8.1 `npm run lint` (tsc --noEmit + eslint, no `any` casts — model
  `derived_from`, LLM response parsing, and cluster types properly).
- [x] 8.2 `npm test` — all new and existing suites pass.
- [x] 8.3 Update `README.md` § MCP surface / config reference (new
  ...
  `README.zh-CN.md`.
- [x] 8.4 Update `AGENTS.md` if the config-vs-environment section needs a note
  (it does not — no new env var, per task 1.7).
- [x] 8.5 Bump `package.json` `version` (user-visible: new CLI command, new config
