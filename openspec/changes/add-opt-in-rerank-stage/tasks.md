...
- [ ] 1.1 Add a `search.rerank` block to the Zod config schema
...
- [ ] 1.2 Confirm no other config consumer needs updating — check for exhaustive
...
- [ ] 2.1 Create `src/rerank/index.ts` with a `RerankProvider` interface —
...
- [ ] 2.2 Implement `OpenAiRerankProvider`: one batched chat-completions POST per
...
- [ ] 2.3 Apply `AbortController`-based timeout using `config.search.rerank.timeout_ms`.
- [ ] 2.4 Validate the response: parse with a Zod schema
...
- [ ] 2.5 Add a `DegradedRerankProvider` (or equivalent no-op) returned when
...
- [ ] 3.1 In `src/index.ts`, next to `embeddingBreaker`/`qdrantBreaker`
  (lines 63-68), construct `const rerankBreaker = new CircuitBreaker({ ...breakerOptions, key: 'rerank', logger })`.
- [ ] 3.2 Instantiate a `RerankProvider` only when `config.search.rerank.enabled` is
...
- [ ] 3.3 Pass the (possibly undefined, when disabled) `RerankProvider` into
  `new SearchService(config, storage, embedding, metrics, logger)`
  (`src/index.ts:104`) as a new optional constructor parameter.
- [ ] 3.4 Add `rerankBreaker` to the `HealthService` breakers map
...
- [ ] 4.1 Add `SearchService.rerank(query: string, results: SearchResult[], poolSize: number): Promise<SearchResult[]>`
...
- [ ] 4.2 Add `rerank_score?: number` to the `SearchResult` interface
...
- [ ] 4.3 In `handleRecall` (`src/tools/index.ts:156-205`): when
...
- [ ] 4.4 Wrap the `SearchService.rerank` call's provider interaction in try/catch:
  ...
  failed.
- [ ] 4.5 Confirm `handleSearch` (`src/tools/index.ts:224-237`) and
...
- [ ] 5.1 `src/rerank/index.test.ts` (new): `OpenAiRerankProvider` sends the expected
  ...
  malformed JSON.
- [ ] 5.2 `src/search/index.test.ts`: `SearchService.rerank` replaces `score` and sets
...
- [ ] 5.3 `src/tools/index.test.ts`: `handleRecall` with `search.rerank.enabled: true`
...
- [ ] 5.4 `src/tools/index.test.ts`: rerank provider failure (mocked rejection) does
  not throw from `handleRecall`, increments `search_rerank_degraded`, and returns the
  pre-rerank ordering.
- [ ] 5.5 `src/config/index.test.ts`: `search.rerank` defaults parse
...
- [ ] 6.1 Document the `search.rerank` config block in `README.md`'s Configuration
...
- [ ] 6.2 Add `BHGBRAIN_RERANK_API_KEY` to the Environment Variables table
...
- [ ] 6.3 Add a `BHGBRAIN_RERANK_API_KEY` entry to `.env.example`, modeled on the
  existing `BHGBRAIN_EXTRACTION_API_KEY` block (`.env.example:26-28`).
- [ ] 6.4 Mirror 6.1/6.2 into `README.de.md`, `README.es.md`, `README.fr.md`,
  `README.zh-CN.md`, section-for-section.
- [ ] 6.5 Bump `package.json` `version` (currently `1.11.0` — this is a user-visible
  change: new tool behavior when opted in, new env var, new config fields).

## 7. Validation

- [ ] 7.1 `npm run lint` (type check + eslint, including
