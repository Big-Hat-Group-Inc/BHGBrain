...
- [ ] 1.1 Add `src/eval/fake-qdrant-client.ts`: a `FakeQdrantClient` implementing the
...
- [ ] 1.2 Add `src/eval/fixture-embedding-provider.ts`: an `EmbeddingProvider`
...
- [ ] 1.3 Unit-test both fixtures directly (`src/eval/fake-qdrant-client.test.ts`,
...
- [ ] 2.1 Author `src/eval/fixtures/corpus.json`: ~150–200 seed memories shaped like
...
- [ ] 2.2 Author `src/eval/fixtures/golden-set.json`: 50 entries of
...
- [ ] 2.3 Add a load-time consistency check (used by both the harness and a small
...
- [ ] 3.1 Add `src/eval/harness.ts` exporting `seedFixtureStore()`: builds a `BrainConfig`
  ...
  checks run for real.
- [ ] 3.2 Add `runGoldenSet(storage, config)` in `src/eval/harness.ts`: constructs a
...
- [ ] 3.3 Add `scoreResults(perQueryRanks)` computing recall@1/@5/@10 and MRR@10 per the
...
- [ ] 3.4 Add `src/eval/harness.test.ts`: seeding populates the expected number of rows
...
- [ ] 4.1 Add `src/eval/golden-set.test.ts`: in `beforeAll`, seed the fixture store
...
- [ ] 4.2 Assert `aggregate.recall5 >= FLOOR_RECALL_5` and `aggregate.mrr10 >=
...
- [ ] 4.3 Confirm no `.github/workflows/ci.yml` changes are needed: `vitest.config.ts`'s
...
- [ ] 5.1 Add `src/eval/run.ts`: a small CLI entry point (`tsx`-run, mirroring
  ...
  spec's job, task 4.2).
- [ ] 5.2 Add `"eval": "tsx src/eval/run.ts"` to `package.json` `scripts`
  (`package.json:16-26`), alongside the existing `dev`/`test`/`lint` entries.

## 6. Docs

- [ ] 6.1 Add the `npm run eval` line to `AGENTS.md`'s "Essential Commands" section
  (`AGENTS.md:19-33`), grouped under a new `# Evaluation` comment near the `Testing`
  block.
- [ ] 6.2 No `README.md`/translation changes: this is dev/CI tooling with no MCP
...
- [ ] 7.1 `npm run lint` passes (`tsc --noEmit` + `eslint src`, both cover `src/eval/**`
...
- [ ] 7.2 `npm test` passes, including `src/eval/golden-set.test.ts` at or above its
...
- [ ] 7.3 `npm run eval` runs standalone and prints a complete report for all 50
