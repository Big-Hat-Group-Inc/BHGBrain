...
- [ ] 6.1 RECONCILE the section count to the canonical value of **10** (code, tests, and the
  ...
  - [ ] 6.1.1 This proposal's spec (`specs/bootstrap-session/spec.md`): "all 12 sections" →
        "all 10 sections" everywhere, and the out-of-range scenario bound.
  - [ ] 6.1.2 This proposal's design.md and proposal.md narrative references to "12-section".
  - [ ] 6.1.3 Tasks 1.1, 1.2, and 5.1 above (currently "12-section" / "all 12 sections").
  - [ ] 6.1.4 The Zod validator's `section` bound — confirm it caps at `TOTAL_SECTIONS` (10),
        matching the corrected "section outside 1–10 → INVALID_INPUT" scenario (it must NOT
        accept 11–12).
  - [ ] 6.1.5 The `import` tool description / `format` enum docstring in
        `src/tools/schemas.ts` (still says "12-section bootstrap format") → "10-section".
  - [ ] 6.1.6 The `bulk-profile-import` parser (`src/pipeline/parser.ts`): it currently
  ...
  - [ ] 6.1.7 Keep `README.md` "MCP Tools Reference" in sync per CLAUDE.md.

- [ ] 6.2 BUG (Finding 2): `reset` clears the tracked `memory_ids` from SQLite BEFORE the
  ...
  - [ ] 6.2.1 Add a failure-path test: make `deleteMemory` reject mid-reset and assert the
