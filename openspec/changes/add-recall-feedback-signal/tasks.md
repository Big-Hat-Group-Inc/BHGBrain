...
- [x] 1.1 Add a `recall_feedback` table to `SCHEMA_SQL` in `src/storage/sqlite.ts`
...
- [x] 1.2 Add `RecallFeedbackEntry` to `src/domain/types.ts` after `ArchiveRecord`
  ...
  ```
- [x] 1.3 Add `recordFeedback(entry: RecallFeedbackEntry): void` to the sqlite store
  ...
  self-flush.
- [x] 1.4 Do **not** add a corresponding `logAudit` call — `recall_feedback` is a
  separate, non-mutating event stream (design.md "Not written to audit_log").

## 2. Domain schema

- [x] 2.1 Add `FeedbackInputSchema` to `src/domain/schemas.ts` after
  ...
  ```
- [x] 2.2 Add `export type FeedbackInput = z.infer<typeof FeedbackInputSchema>;` next
  to the other input-type exports (`src/domain/schemas.ts:137-147`).
- [x] 2.3 Add coverage in `src/domain/schemas.test.ts` (alongside the existing
...
- [x] 3.1 Add a `feedback` entry to `MCP_TOOL_DEFINITIONS` in `src/tools/schemas.ts`,
  ...
  ```
- [x] 3.2 Import `FeedbackInputSchema` in `src/tools/index.ts`'s import block
  ...
  ```
- [x] 3.3 Add the dispatcher case in `handleTool`'s switch (`src/tools/index.ts:116-127`,
  next to `case 'review': return handleReview(...)` at line 126):
  `case 'feedback': return handleFeedback(ctx, args, clientId, logCtx);`

## 4. Tests

- [x] 4.1 In `src/tools/index.test.ts` (co-located with the other `handle*` tests, see
...
- [x] 4.2 In `src/storage/sqlite.test.ts`, cover `recordFeedback` directly: row
...
- [x] 4.3 Confirm (via a targeted assertion, not new production code) that recording
...
- [x] 5.1 Add a `### \`feedback\` - Record Recall Usefulness` section to `README.md`'s
...
- [x] 5.2 Mirror the same section into `README.de.md`, `README.es.md`, `README.fr.md`,
  ...
  in all five or none.
- [x] 5.3 Update `CLAUDE.md`'s canonical tool list (`CLAUDE.md:16-17`) to append
  `feedback` to the `Registered:` list.
- [x] 5.4 Bump `package.json` `version` (`package.json:3`, currently `1.11.0`) per a
  user-visible MCP surface change.
- [x] 5.5 `.env.example` and `AGENTS.md`: no changes — this adds no environment
  variable and no config-vs-env resolution behavior.

## 6. Validation

- [x] 6.1 `npm run lint` (tsc --noEmit + eslint src) passes.
- [x] 6.2 `npm test` passes, including the new tests from section 4.
- [x] 6.3 Confirm README ×5 stayed in sync (spot-check the new section renders
