...
- [ ] 1.1 Add a test in `src/search/index.test.ts` where the ranked input is e.g.
...
- [ ] 1.2 Add a test covering the cross-device Qdrant-fallback branch (`src/search/index.ts`
...
- [ ] 2.1 Make the `fullyIncluded` comparison in `src/resources/index.ts` use consistent
...
- [ ] 2.2 Ensure the budget guard and truncation flag are computed consistently so the
  character budget is honored exactly near the boundary for multibyte/astral content.
- [ ] 2.3 Add a test in `src/resources/index.test.ts` with multibyte/astral category content
...
- [ ] 3.1 Rework `recordAccessBatch` in `src/storage/sqlite.ts` to reuse a single prepared
...
- [ ] 3.2 Confirm existing access-recording behavior is unchanged (access_count,
...
- [ ] 4.1 In `src/search/index.ts` (~194-205), validate/parse the Qdrant payload shape
...
- [ ] 4.2 Fall back to the existing defaults when a field is missing or malformed, so a
