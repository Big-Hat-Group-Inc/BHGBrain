...
- [x] 1.1 Create `src/embedding/request.ts`: extract the Azure provider's per-attempt
...
- [x] 1.2 Wire `OpenAIEmbeddingProvider` (`src/embedding/index.ts:37-121`) to the
...
- [x] 1.3 Keep `OpenAIEmbeddingProvider.healthCheck` (`src/embedding/index.ts:77-85`)
  ...
  at `README.md:3178`.
- [x] 1.4 Refactor `AzureFoundryEmbeddingProvider` onto the shared helper with no
...
- [x] 1.5 Tests (`src/embedding/index.test.ts` + a co-located test for the helper):
...
- [x] 2.1 Add a private `ensuredCollections: Set<string>` to `QdrantStore`
...
- [x] 2.2 Invalidate the entry in `deleteCollection` (`src/storage/qdrant.ts:332-343`)
  and clear the whole set in `clearManagedCollections` (`:423-441`).
- [x] 2.3 In `upsert` (`src/storage/qdrant.ts:106-125`): on a not-found error from
  `client.upsert`, drop the memoized entry, re-run `ensureCollection`, retry the
  upsert exactly once; a second not-found propagates.
- [x] 2.4 Tests (`src/storage/qdrant.test.ts`): second `upsert` to the same collection
...
- [x] 3.1 Cache `listAllCollections` (`src/storage/qdrant.ts:354-359`) results for a
...
- [x] 3.2 Confirm namespace-wide `search` (`src/storage/qdrant.ts:199`) and the
...
- [x] 3.3 Tests: two namespace-wide searches within the TTL issue one
...
- [x] 4.1 Implement a ~256-entry LRU keyed by
...
- [x] 4.2 Verify write-path call sites remain uncached (direct provider calls):
...
- [x] 4.3 Tests (`src/search/index.test.ts`): repeat identical query embeds once;
...
- [x] 5.1 In `hybridSearch` (`src/search/index.ts:192-230`), dispatch the embed
...
- [x] 5.2 Tests: an embed rejection that settles before the fulltext scan finishes
...
- [x] 6.1 Audit `README.md` for accuracy: `README.md:3178` (health-probe timeout
  ...
  (Audited: both passages already describe the intended cross-provider behavior
  with no OpenAI-specific caveat, so no text changed and no translation sync
  was needed.)
- [x] 6.2 Bump `package.json` version (user-visible: OpenAI embedding calls now honor
  documented timeout/retry; hot-path latency changes).
- [x] 6.3 `npm run lint` passes (tsc + eslint, no `any` casts in the new helper/caches).
- [x] 6.4 `npm test` passes.
