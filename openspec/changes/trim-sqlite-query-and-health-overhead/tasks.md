...
- [ ] 1.1 Append to `SCHEMA_SQL` (`src/storage/sqlite.ts:203-214`):
...
- [ ] 1.2 Add `EXPLAIN QUERY PLAN` tests in `src/storage/sqlite.test.ts` asserting
...
- [ ] 1.3 Migration test: open a DB image created with the old schema (build it in
...
- [ ] 2.1 Add `deleteMemoriesByIds(ids: string[]): number` to `SqliteStorage`
...
- [ ] 2.2 Replace the per-row loop in `StorageManager.deleteMemories`
...
- [ ] 2.3 Add `listMemoryIds(): Set<string>` (or reuse the shape of
...
- [ ] 2.4 Convert hydration to per-collection `BEGIN`/`COMMIT` with a per-point
  ...
  `StorageManager`.
- [ ] 2.5 Tests: batch delete removes rows from both tables and returns the exact
...
- [ ] 3.1 Add a `Map<string, Statement>` cache to `SqliteStore`, used by a
...
- [ ] 3.2 Free every cached statement and clear the map in `close()`
...
- [ ] 3.3 Tests: repeated calls reuse one statement (spy on `db.prepare`), results
...
- [ ] 4.1 Add to the retention config block (`src/config/index.ts:121-145`):
...
- [ ] 4.2 Add `pruneAuditLog(maxEntries: number): number` to `SqliteStore`: delete
...
- [ ] 4.3 Add `pruneRevisions(maxPerMemory: number): number`: for each `memory_id`
...
- [ ] 4.4 Invoke both from `RetentionService.runGc`'s destructive phase (before the
...
- [ ] 4.5 Tests: caps enforced (newest audit rows and highest revisions survive),
...
- [ ] 5.1 In `HealthService.check()` (`src/health/index.ts:24-61`), compute
  ...
  count as parameters.
- [ ] 5.2 Cache the SQLite stats block — `memory_count`, `db_size_bytes`, tier
...
- [ ] 5.3 Tests: one poll invokes each aggregate exactly once (spies on the sqlite
...
- [ ] 6.1 Rewrite `generateSummary` (`src/domain/normalize.ts:15-19`) to find the
...
- [ ] 6.2 Extend `src/domain/normalize.test.ts`: single-line content, multi-line
  content, empty string, and a first line longer than `maxLen` all produce output
  identical to the previous implementation.

## 7. Docs and validation

- [ ] 7.1 Document `retention.audit_log_max_entries` and
...
- [ ] 7.2 `npm run lint` passes (tsc + eslint, no `any` casts).
- [ ] 7.3 `npm test` passes.
