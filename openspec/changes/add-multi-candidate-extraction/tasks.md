...
- [x] 1.1 In `src/config/index.ts`'s `pipeline` object (currently
...
- [x] 1.2 Add a `getExtractionBreakerKey()`-style constant (or inline `'extraction'`
...
- [x] 2.1 Create `src/pipeline/extraction.ts` mirroring `src/embedding/index.ts`'s
...
- [x] 2.2 Implement `LlmExtractionProvider`: chat-completions call to
...
- [x] 2.3 Define and apply a Zod response schema (`candidates: z.array(z.object({
...
- [x] 2.4 Apply `extraction_max_candidates`: candidates beyond the cap are dropped (not
...
- [x] 2.5 Implement `NoopExtractionProvider` (always returns `null`, no network call)
...
- [x] 2.6 Implement `warnIfExtractionDegraded(provider, config, logger)`, called once at
...
- [x] 3.1 In `src/pipeline/index.ts`, add an optional `extraction?: ExtractionProvider`
...
- [x] 3.2 Change `extract()` (`src/pipeline/index.ts:61-80`) to `async`. When
...
- [x] 3.3 Update `process()` (`src/pipeline/index.ts:29-59`) to `await this.extract(...)`
...
- [x] 3.4 Wire the extraction provider and breaker into both call sites: `src/index.ts`
...
- [x] 4.1 In `src/health/metrics.ts`, add (mirroring `embedding_embed_batch_ms`'s
  ...
  Note: `MetricsCollector` (`src/health/metrics.ts`) is a fully generic, string-keyed
  collector with no metric-name registry — every existing metric (e.g.
  `embedding_embed_batch_ms`) is just a call-site string, nothing is "added" to the
  file itself to define a new one. This task is satisfied by the call sites in
  `src/pipeline/extraction.ts` (`extraction_ms` histogram,
  `extraction_fallback_total`/`extraction_candidates_truncated_total` counters) and
  `src/pipeline/index.ts` (`extraction_candidate_failed_total` counter).

## 5. Tests

- [x] 5.1 `src/pipeline/extraction.test.ts`: `LlmExtractionProvider` returns validated
...
- [x] 5.2 `src/pipeline/index.test.ts`: `extract()` calls the extraction provider only
...
- [x] 5.3 `src/pipeline/index.test.ts`: partial-batch failure — with 3 candidates where
  ...
  `process()` call.
- [x] 5.4 `src/config/index.test.ts` (or equivalent): `extraction_enabled` defaults to
  `false`; the three new fields validate their bounds and defaults (120 / 6 / 4000).
- [x] 5.5 `src/tools/index.test.ts`: `handleRemember` returns an array (not collapsed)
...
- [x] 6.1 `README.md`: update the `pipeline` config block (`README.md:491-501`) to
...
- [x] 6.2 Propagate the same README changes to `README.de.md`, `README.es.md`,
  `README.fr.md`, `README.zh-CN.md` (section-for-section mirror, per `CLAUDE.md`).
- [x] 6.3 `AGENTS.md`: no structural change expected (config-vs-environment section
  already describes `extraction_model_env`'s role correctly), but re-read it after
  implementation to confirm no drift. Re-read: `AGENTS.md:143` still accurately
  describes `pipeline.extraction_model_env` defaulting to `BHGBRAIN_EXTRACTION_API_KEY`
  — no change needed.
- [x] 6.4 Bump `package.json` `version` (user-visible: new config fields, changed
  default, new response shape reachable for the first time). Bumped 1.13.0 → 1.14.0.

## 7. Spec sync

- [x] 7.1 Confirm `openspec/changes/add-multi-candidate-extraction/specs/write-decision-pipeline/spec.md`
  ...
  Reviewed against the implementation: all scenarios (length gate, disabled default,
  model-failure fallback, candidate-cap truncation, timeout/circuit-breaker fallback,
  key-resolution fallback chain, partial-batch failure, breaker excluded from
  aggregate health) match the shipped code as written; no edits needed.

## 8. Validation

- [x] 8.1 `npm run lint` (tsc --noEmit + eslint, including `no-explicit-any` on the new
  extraction response parsing — validate with Zod, never cast through `any`).
- [x] 8.2 `npm test`.
- [x] 8.3 Confirm README ×5 stayed in sync (task 6.2) and `package.json` version bumped
