...
- [ ] 1.1 Add four fields to the `deduplication` object in `src/config/index.ts`
...
- [ ] 1.2 Add a doc comment above the block explaining the corroboration formula,
  mirroring the style of the `search.ranking` comment at `src/config/index.ts:162-165`.

## 2. Classifier logic

- [ ] 2.1 In `classifyOperation` (`src/pipeline/index.ts:291-313`), slice `similar` to
  ...
  three `if` blocks.
- [ ] 2.2 After the existing three threshold checks and before the final
...
- [ ] 2.3 When the corroboration branch fires, call `this.logger?.warn({ event:
...
- [ ] 2.4 Confirm by inspection that no changes are needed to the `decide()` call site
...
- [ ] 3.1 Corroboration triggers UPDATE: mock `storage.qdrant.searchSimilar` to return
...
- [ ] 3.2 Corroboration count not met: only 1 candidate qualifies within the margin;
  assert the result is still `ADD` (default `corroboration_count: 2` not reached).
- [ ] 3.3 Kill-switch test: with `deduplication.corroboration_enabled: false` and the
  same 3+-candidate cluster from 3.1, assert the result is `ADD` (pre-widening
  behavior preserved).
- [ ] 3.4 Window-size regression test: with `deduplication.candidate_window: 1` and the
...
- [ ] 3.5 Regression check: run the existing NOOP/UPDATE(direct)/DELETE/ADD
...
- [ ] 3.6 Logging assertions: `logger.warn` is called with a `corroborated_dedup`
...
- [ ] 4.1 `README.md`: update the Phase 2 mermaid diagram (`:1168-1204`, node `G`/`H`
...
- [ ] 4.2 Mirror the same changes into `README.de.md`, `README.es.md`, `README.fr.md`,
  `README.zh-CN.md` — CLAUDE.md requires a user-facing `README.md` change to land in
  all five translations or none.
- [ ] 4.3 Bump `package.json` `version` (currently `1.11.0`) for the new config surface
  and behavior change.
- [ ] 4.4 Run `npm run lint && npm test`; fix any fallout before considering the change
  complete.
