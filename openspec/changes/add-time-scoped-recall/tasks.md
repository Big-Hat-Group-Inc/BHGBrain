...
- [ ] 1.1 Extend `RecallFilter` (`src/domain/types.ts:23`) with `after?: string` /
...
- [ ] 1.2 Add `after: z.string().datetime().optional()` / `before:
...
- [ ] 1.3 Add the same two fields + refine to `SearchInputSchema`
  (`src/domain/schemas.ts:52-62`).
- [ ] 1.4 Add `after`/`before` to the `recall` and `search` JSON schemas
...
- [ ] 2.1 In `QdrantStore.search` (`src/storage/qdrant.ts:160-189`), append `{ key:
...
- [ ] 2.2 Add a `datetime` payload index on `created_at` in `ensureCollection`
...
- [ ] 3.1 In `SqliteStore.fullTextSearch` (`src/storage/sqlite.ts:726-776`), append
...
- [ ] 4.1 `handleRecall` (`src/tools/index.ts:156-205`): extend the existing filter
...
- [ ] 4.2 `handleSearch` (`src/tools/index.ts:224-237`): replace the hardcoded
...
- [ ] 4.3 Confirm `SearchService.search`'s existing `filter?: RecallFilter` parameter
...
- [ ] 5.1 `src/storage/qdrant.test.ts`: assert the `must` clause shape (a `range`
...
- [ ] 5.2 `src/storage/qdrant.test.ts`: assert `ensureCollection` creates the
  `created_at` `datetime` index, including the retroactive (already-existing
  collection) path.
- [ ] 5.3 `src/storage/sqlite.test.ts`: assert `fullTextSearch` excludes memories
  outside an `after`/`before` window and includes ones inside it, including boundary
  (exactly-equal-to-bound, inclusive) cases.
- [ ] 5.4 `src/tools/index.test.ts`: filtered `recall` returns `limit` in-window
...
- [ ] 5.5 `src/tools/index.test.ts`: `search` with `after`/`before` builds and passes a
...
- [ ] 5.6 `src/tools/index.test.ts`: `search_zero_after_filter` increments on defensive
...
- [ ] 5.7 `src/domain/schemas.ts` validation tests (or co-located): `after > before` is
...
- [ ] 6.1 Update `README.md` § MCP Tools Reference for `recall` (`README.md:2363-2400`)
...
- [ ] 6.2 Apply the same documentation updates to `README.de.md`, `README.es.md`,
  `README.fr.md`, `README.zh-CN.md`, keeping section structure aligned with
  `README.md`.
- [ ] 6.3 Bump `package.json` `version` (currently `1.11.0`) for the user-visible
  parameter additions.
- [ ] 6.4 Run `npm run lint` (`tsc --noEmit` + `eslint src`) and `npm test`; both must
