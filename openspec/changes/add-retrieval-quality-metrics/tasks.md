...
- [ ] 1.1 Extend `incCounter` (`src/health/metrics.ts:78`) to
  ...
  (`:61-65`).
- [ ] 1.2 Change `counters` (`src/health/metrics.ts:69`) from `Map<string, number>` to
...
- [ ] 1.3 Update the counters loop in `getMetrics()` (`:104-106`) to emit `labels` on
  each counter `MetricEntry`, matching how the histogram loop (`:107-122`) already does.
- [ ] 1.4 Tests in `src/health/metrics.test.ts`: a labeled `incCounter` call accumulates
...
- [ ] 2.1 In `SearchService.search()` (`src/search/index.ts`), after the mode `switch`
...
- [ ] 2.2 Tests in `src/search/index.test.ts`: for each of `semantic`, `fulltext`,
...
- [ ] 3.1 Add a `{ namespace }` label to the `search_embedding_degraded` call in
  `hybridSearch` (`src/search/index.ts:231`), using the `namespace` parameter already in
  scope (`:194`).
- [ ] 3.2 Update the existing assertion pinning the old unlabeled call shape
...
- [ ] 3.3 Add a test with a second, distinct namespace confirming
...
- [ ] 4.1 Add rows to the `README.md` § Metrics table (`:1911-1924`) for
  ...
  README.
- [ ] 4.2 Apply the identical table additions/updates to `README.de.md`, `README.es.md`,
  `README.fr.md`, `README.zh-CN.md`, section-for-section with `README.md`.
- [ ] 4.3 Bump the `version` field in `package.json` (user-visible `/metrics` output
  change).
- [ ] 4.4 No `.env.example` changes required — no new environment variables are
  introduced (confirm during review).

## 5. Validation

- [ ] 5.1 `npm run lint` (`tsc --noEmit` + `eslint src`) passes.
- [ ] 5.2 `npm test` passes, including the new/updated tests in
  `src/health/metrics.test.ts` and `src/search/index.test.ts`.
- [ ] 5.3 Confirm README ×5 stayed in sync: grep for `search_result_count`,
