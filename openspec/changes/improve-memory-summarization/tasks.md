...
- [x] 1.1 Add to the `pipeline` object in the Zod config schema
...
- [x] 1.2 Leave `auto_summarize` (`src/config/index.ts:217`) as-is — it becomes
  load-bearing via task 3.x, no schema change needed there.
- [x] 1.3 Add `pipeline.summarization_model_env`'s referenced var to
...
- [x] 2.1 Create `src/domain/summarize.ts`. Implement `extractiveSummary(content:
...
- [x] 2.2 Add `summarizeContent(content: string, config: BrainConfig, provider?:
...
- [x] 2.3 Unit tests in `src/domain/summarize.test.ts`: extractive picks the
...
- [x] 2.4 Update `src/domain/normalize.test.ts:46-48` (`generateSummary('first\n
...
- [x] 3.1 Define `SummarizationProvider` interface (`summarize(content: string,
  ...
  `AbortController`.
- [x] 3.2 Add `createSummarizationProvider(config, options?: { breaker?, metrics? }):
...
- [x] 3.3 Wire into `src/index.ts`: construct the provider after `embedding`
  ...
  (missing credentials).
- [x] 3.4 Tests for `OpenAISummarizationProvider`/`DegradedSummarizationProvider` in
...
- [x] 4.1 `src/pipeline/index.ts`: thread an optional `summarizer?:
...
- [x] 4.2 `src/pipeline/index.ts:359` (`deterministicFallback`): replace
...
- [x] 4.3 `src/storage/index.ts:264` (`revertMemory`): add a new optional
...
- [x] 4.4 Update existing call-site instantiations of `WritePipeline`/
...
- [x] 5.1 `src/pipeline/index.test.ts`: ADD/UPDATE/DELETE outcomes carry an
...
- [x] 5.2 `src/pipeline/index.test.ts`: with a mocked `SummarizationProvider` that
  rejects, the write still succeeds and the persisted summary is the extractive
  fallback, not an error.
- [x] 5.3 `src/pipeline/index.test.ts`: with a mocked `SummarizationProvider` that
  resolves, the persisted summary is the (truncated) provider output.
- [x] 5.4 `src/storage/index.test.ts` (`revertMemory` describe block, ~line 261+):
  reverting to a multi-sentence revision produces an extractive summary of that
  revision's content, not its first line.
- [x] 5.5 Regression: `fullTextSearch` tests (`src/storage/sqlite.test.ts`) are
...
- [x] 6.1 `npm run lint` (tsc --noEmit + eslint src) passes — no `any` casts,
  especially around the `AbortController`/fetch typing in
  `src/summarization/index.ts`.
- [x] 6.2 `npm test` passes, including the new `src/domain/summarize.test.ts` and
  `src/summarization/index.test.ts` suites.
- [x] 6.3 Update `README.md`'s pipeline config block (`README.md:491-504`): document
...
- [x] 6.4 Bump `package.json` `version` (currently `1.11.0`) — user-visible behavior
