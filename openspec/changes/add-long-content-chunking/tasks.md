...
- [ ] 1.1 Add `long_content_threshold_chars` to the `pipeline` block in the Zod config
...
- [ ] 1.2 Confirm no `.env.example` change is needed (no new env var — this is a
...
- [ ] 2.1 In `handleRemember` (`src/tools/index.ts:133-154`), after
...
- [ ] 2.2 Do not add this guard to `WritePipeline.process`
...
- [ ] 2.3 Content at exactly the threshold is accepted (only content strictly greater
...
- [ ] 3.1 `src/tools/index.test.ts`: `remember` with content over the default 8,000
...
- [ ] 3.2 `src/tools/index.test.ts`: `remember` with content exactly at the threshold,
...
- [ ] 4.1 `README.md` § `remember` (around line 2294-2310): add a row or note for the
  ...
  callers see.
- [ ] 4.2 `README.md` § `import` (around line 2656-2687): add a short cross-reference
...
- [ ] 4.4 Bump `package.json` `version` (currently `1.11.0`) — this is a user-visible,
  behavior-changing addition (existing long `remember` calls that used to succeed now
  fail).

## 5. Validation

- [ ] 5.1 `npm run lint` (tsc --noEmit + eslint, no `any` casts).
- [ ] 5.2 `npm test` — full suite green, including the new tests from section 3.
