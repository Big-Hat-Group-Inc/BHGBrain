...
- [ ] 0.1 Re-check `openspec/changes/` for a landed or in-progress proposal that
  ...
  this is still true before choosing a path.
- [ ] 0.2 If such a client now exists: skip task 1's client build and instead import
...
- [ ] 1.1 Add `src/pipeline/entailment.ts` exporting a single function, e.g.
  ...
  labels.
- [ ] 1.2 Enforce `timeout_ms` (new config field, task 2.1) via `AbortController`,
  ...
  `contradict`.
- [ ] 1.3 Unit tests for `entailment.ts` (`src/pipeline/entailment.test.ts`): each of
...
- [ ] 2.1 Add `pipeline.contradiction_detection` to the Zod schema
  ...
  Depends on: none.
- [ ] 2.2 Add a schema test asserting the new block's defaults (`enabled: false`,
...
- [ ] 3.1 In `classifyOperation` (`src/pipeline/index.ts:291-314`), keep the existing
...
- [ ] 3.2 `classifyOperation` currently is synchronous
  ...
  Depends on: 3.1.
- [ ] 3.3 Wire the fail-open path: wrap the entailment call in try/catch (or use the
  ...
  Depends on: 3.1.
- [ ] 3.4 Confirm the `contradict` path produces an identical `WriteResult` shape to
...
- [ ] 4.1 `classifyOperation`/`decide` test: candidate in the UPDATE band, regex does
...
- [ ] 4.2 Same setup with entailment mock returning `agree` and separately `refine` →
  expect `UPDATE`, existing merge behavior unchanged.
- [ ] 4.3 Regex-match case: candidate matches `detectsInvalidation` → entailment mock
  must NOT be called (assert zero invocations) → `DELETE`, same as pre-change
  behavior.
- [ ] 4.4 `contradiction_detection.enabled: false` (default) → entailment mock must
  NOT be called regardless of band/content → behavior identical to pre-change
  pipeline.
- [ ] 4.5 Fail-open test: entailment mock throws / times out → `UPDATE` (not an
  error surfaced to the caller, not a dropped write), degraded-path log emitted.
- [ ] 4.6 Regression: existing `src/pipeline/index.test.ts` NOOP/UPDATE/ADD/regex-
...
- [ ] 5.1 Update the write-pipeline decision table in `README.md` (around
  ...
  Depends on: 3, 4.
- [ ] 5.2 Mirror the same README changes into `README.de.md`, `README.es.md`,
  `README.fr.md`, `README.zh-CN.md` — section-for-section per `CLAUDE.md`'s
  translation-sync rule.
  Depends on: 5.1.
- [ ] 5.3 Update `AGENTS.md`'s "Config vs. environment" section only if this proposal
  ...
  `add-composite-recall-ranking/tasks.md` task 1.2.
  Depends on: 5.1.
- [ ] 5.4 Bump `package.json` `version` (user-visible behavior change once enabled).
  Depends on: 5.1.
- [ ] 5.5 Run `npm run lint` (tsc --noEmit + eslint, no `any` casts) and `npm test`;
  fix until both pass clean.
  Depends on: 1, 2, 3, 4.
