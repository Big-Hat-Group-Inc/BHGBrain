...
- [x] 1.1 Add a `search.mmr` block to the Zod config schema (`src/config/index.ts`,
...
- [x] 1.2 Document the block in `.env.example`'s config-reference comments only if
...
- [x] 2.1 Extract `cosineSimilarity` (currently private to `ResourceHandler`,
...
- [x] 2.2 Update `src/resources/index.ts` to import and use the shared function in
...
- [x] 3.1 Add a `withVectors` parameter to `semanticSearch`
  ...
  requested.
- [x] 3.2 In the public `search()` method (`src/search/index.ts:82-135`), compute
...
- [x] 4.1 Add `mmrRerank(results: SearchResult[], lambda: number): SearchResult[]` to
...
- [x] 4.2 In `search()`, after the mode switch produces `results` and before the
...
- [x] 4.3 Before `search()` returns, clear the transient `.vector` field on every
...
- [x] 5.1 In `handleRecall` (`src/tools/index.ts:170-178`), replace the fixed
...
- [x] 5.2 In `handleSearch` (`src/tools/index.ts:224-237`), fetch a wider pool using
...
- [x] 6.1 Diversity test: seed a near-duplicate high-relevance cluster plus one
...
- [x] 6.2 Lambda test: `lambda: 1` reduces to (near) pure composite-relevance
...
- [x] 6.3 Fulltext no-op test: `search` with `mode: 'fulltext'` is unaffected by
  `search.mmr.enabled` (no vectors exist to diversify against).
- [x] 6.4 Scale-normalization test: hybrid-mode (tiny RRF-scale composite scores) and
...
- [x] 6.5 `min_score` regression: `recall`'s `min_score` filter still applies to
...
- [x] 6.6 Vector-leakage regression: `search()`'s JSON-serializable output never
...
- [x] 6.7 `memory://inject/{hint}` regression: near-duplicate suppression output and
  its existing test suite are unaffected by the `cosineSimilarity` extraction (task
  2) — same inputs produce the same outputs.

## 7. Docs

- [x] 7.1 Document `search.mmr` config and the full ranking pipeline order
...
- [x] 7.2 Bump `package.json` `version` (user-visible ranking behavior change).
- [x] 7.3 `npm run lint` and `npm test` pass.
