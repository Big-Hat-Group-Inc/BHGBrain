...
- [x] 1.1 Add `search.query_expansion` to the Zod config schema
  ...
  int, default `3000`).
- [x] 1.2 Document the block in `README.md`'s config reference (near the existing
...
- [x] 2.1 Create `src/search/query-expansion.ts` with a small fixed English stopword
...
- [x] 2.2 Add `buildVariants(query: string, config: BrainConfig['search']['query_expansion'], llmVariants?: string[]): string[]` in the
  ...
  `max_variants`.
- [x] 2.3 Unit tests in `src/search/query-expansion.test.ts`: stopword stripping
...
- [x] 3.1 In `semanticSearch` (`src/search/index.ts:137-168`), when
...
- [x] 3.2 In `hybridSearch` (`src/search/index.ts:192-283`), apply the same
...
- [x] 3.3 Add a `recordHistogram('search_query_expansion_variant_count', n)` call
  ...
  actually used.
- [x] 3.4 Confirm `handleRecall` (`src/tools/index.ts:156-205`) and `handleSearch`
...
- [x] 4.1 `semanticSearch` test: a query whose keyword-stripped variant surfaces a
...
- [x] 4.2 Merge test: a memory id present in both variants' results keeps its max
  score, not a summed or averaged one.
- [x] 4.3 Limit test: with two variants each returning up to `limit` distinct
  candidates, the final result count is still bounded by `limit`.
- [x] 4.4 Kill-switch test: `search.query_expansion.enabled: false` calls
...
- [x] 4.5 `hybridSearch` test: semantic-leg expansion merges into RRF the same way;
...
- [x] 4.6 Regression: existing `min_score` and composite-ranking tests in
...
- [x] 5.1 Create `LLMQueryExpansionProvider` in `src/search/query-expansion.ts` (or a
...
- [x] 5.2 Implement `generateVariants(query: string, mode: 'paraphrase' | 'hyde', count: number, timeoutMs: number): Promise<string[]>`
...
- [x] 5.3 Wrap the provider's fetch in its own `CircuitBreaker` (`key: 'extraction'`),
  constructed alongside `embeddingBreaker`/`qdrantBreaker` in `src/index.ts:63-65`
  and passed to `SearchService`'s constructor.
- [x] 5.4 Thread an optional `queryExpansion?: LLMQueryExpansionProvider` dependency
  ...
  exactly as phase 1.
- [x] 5.5 Startup-time visibility: if `llm_paraphrase.enabled` is true but no key
...
- [x] 6.1 `generateVariants` unit tests (mocked `fetch`): success returns the
...
- [x] 6.2 Missing-key test: no `extraction_model_env` value and no `OPENAI_API_KEY`
  set → provider reports unconfigured, `SearchService` skips LLM expansion silently
  (no per-call warning, no thrown error).
- [x] 6.3 `OPENAI_API_KEY` fallback test: `extraction_model_env`-named variable unset
  but `OPENAI_API_KEY` set → provider resolves the key and is configured.
- [x] 6.4 Integration test in `src/search/index.test.ts`: with `llm_paraphrase.enabled`
...
- [x] 6.5 Degrade test: mock provider throwing/timing out mid-call still returns a
...
- [x] 7.1 Document `search.query_expansion` (both phases) in `README.md`'s config
  reference section and `README.de.md`, `README.es.md`, `README.fr.md`,
  `README.zh-CN.md`, mirroring section-for-section.
- [x] 7.2 Update the `BHGBRAIN_EXTRACTION_API_KEY` row in `README.md`'s environment
...
- [x] 7.3 Update `AGENTS.md`'s "Config vs. environment" section if the
  `extraction_model_env` bullet (`AGENTS.md:136`) needs to note the new fallback
  behavior.
- [x] 7.4 Bump `package.json` `version` (currently `1.11.0`).
- [x] 7.5 Run `npm run lint && npm test`; both must pass before this change is
  considered done.
