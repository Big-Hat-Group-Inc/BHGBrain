...
- **THEN** the system SHALL create a new session with all 10 sections marked as pending and return the first section's title, questions, and instructions
...
- **WHEN** `bootstrap` is called with `action: "start"` and all 10 sections are complete
...
- **WHEN** `submit` is called with a section number outside 1–10
...
- **THEN** the system SHALL return a list of all 10 sections with their status (pending/complete), memory count per section, total memories stored, and last updated timestamp
...
The system SHALL accept `action: "reset"` with a `section` number, delete all memories created for that section, and mark it as pending. To avoid orphaned vector data, the system SHALL delete the stored memories (including their Qdrant vectors) BEFORE clearing the SQLite `memory_ids` tracking and marking the section pending; if any deletion fails, the tracked `memory_ids` SHALL remain recoverable so the section can be re-reset.
...
#### Scenario: Reset deletes vectors before clearing tracking
- **WHEN** `reset` is called for a complete section
- **THEN** the system SHALL delete the section's memories and their Qdrant vectors before it clears the `memory_ids` column or marks the section pending

#### Scenario: Reset with a failing deletion preserves the recovery list
- **WHEN** `reset` is called for a complete section and deleting one of the tracked memories fails
- **THEN** the system SHALL NOT have cleared the `memory_ids` for that section, so no Qdrant vectors are orphaned without a tracked recovery list and the reset can be retried
...
> Audit reconciliation (2026-06-05): the canonical section count is **10**, matching
> `src/bootstrap/sections.ts` (`BOOTSTRAP_SECTIONS` / `TOTAL_SECTIONS`), the bootstrap and
> import tests, and the Zod validator. This capability had not yet been archived into
> `openspec/specs/`, so the count is corrected directly above rather than via a separate
> MODIFIED block. This reconciliation also resolves the `bulk-profile-import` "12-section"
> drift, since both features share the same section table.
