...
- [x] 1.1 Add `keep_alive_timeout_ms` (default 65000), `headers_timeout_ms`
...
- [x] 1.2 Add a `superRefine` enforcing `headers_timeout_ms >
...
- [x] 2.1 Extract a shared `createShutdown(...)` helper in `src/index.ts` and
...
- [x] 2.2 Arm a 10 s unref'd hard-deadline timer when shutdown starts: on
  expiry, log a `shutdown_timeout` event, run `sqlite.cancelDeferredFlush()` +
  `sqlite.flushIfDirty()` synchronously, and `process.exit(1)`.
- [x] 2.3 Stdio branch (`src/index.ts:104-122`, currently registers no
...
- [x] 2.4 Keep the existing `shuttingDown` re-entrancy guard covering both the
  ...
  > Note (verified 2026-08-30): `migrate-sqlite-to-native-engine` landed
  > before this change and replaced the old sql.js deferred-write buffer with
  > `node:sqlite` + WAL (`synchronous=NORMAL`), so `flushIfDirty`,
  > `scheduleDeferredFlush`, and `cancelDeferredFlush` are now documented
  > no-ops (`src/storage/sqlite.ts:718-739`) — every commit is already durable
  > at commit time, so there is no literal "buffered write" left to lose on
  > `process.exit(0)`. The tasks are still implemented exactly as written
  > (all four methods are called at the documented points) because `close()`
  > still does real, valuable work on shutdown — `PRAGMA wal_checkpoint
  > (TRUNCATE)` folds the WAL back into `brain.db` and removes the `-wal`/
  > `-shm` sidecars, which matters for `docker stop`/SIGKILL races and for
  > any tool that reads the database file directly — and calling the
  > interface's flush methods keeps this code correct if a future storage
  > engine reintroduces real buffering. Not left unchecked: the code change
  > described is correct and complete, only the "buffered write" framing in
  > the proposal is stale.

## 3. JSON error envelope on every HTTP failure path

- [x] 3.1 Add a terminal 4-arg error middleware in `createHttpServer`, after the
...
- [x] 3.2 Guard the resource URI parse: wrap `new URL(uri)` in
  ...
  middleware exists.
- [x] 3.3 Add `ENV NODE_ENV=production` to the runtime stage of the `Dockerfile`
  (ENV block at `Dockerfile:27-29`) as defense in depth.

## 4. Server timeouts

- [x] 4.1 After capturing the listener (`const httpServer = app.listen(...)`,
...
- [x] 5.1 In `createHttpServer` (`src/transport/http.ts:67`, right after
  `express()`): `app.disable('x-powered-by')` and a middleware setting
  `X-Content-Type-Options: nosniff` on every response.
- [x] 5.2 Add `compression` + `@types/compression` to `package.json` and
...
- [x] 6.1 Shutdown-persistence test: a `SqliteStore` with a pending deferred
  ...
  > Note: added in `src/storage/sqlite.test.ts` following the exact call
  > sequence `createShutdown()` uses. See the task 2.x note above —
  > `scheduleDeferredFlush`/`flushIfDirty` are no-ops under the current
  > `node:sqlite` + WAL engine (`migrate-sqlite-to-native-engine` landed
  > first), so this is now equivalent to (and sits next to) the pre-existing
  > "a committed write survives close and reopen without ever calling
  > flush()" durability test; kept as its own test because it exercises the
  > literal call sequence this proposal specifies, not just the outcome.
- [x] 6.2 Error-middleware tests in `src/transport/http.test.ts` (supertest, no
  ...
  generic message only. Also added a direct `resources/index.test.ts` test of
  `ResourceHandler.handle('not-a-url')` itself (task 3.2's actual guard).
- [x] 6.3 Header tests: responses carry no `X-Powered-By` and carry
  `X-Content-Type-Options: nosniff`.
- [x] 6.4 Compression test: a large JSON response with `Accept-Encoding: gzip`
  comes back compressed; a `text/event-stream` response does not.

  > Note: the SSE-vs-everything-else branch is asserted against the exported
  > `compressionFilter` function directly rather than by driving a real
  > `text/event-stream` response through `supertest` end-to-end — an open SSE
  > response never completes on its own, which would hang the test. The large
  > `/metrics` JSON response is still asserted end-to-end for real gzip
  > framing.
- [x] 6.5 Timeout wiring test: with custom `transport.http` timeout config, the
  ...
  > Note: the timeout-application logic in `src/index.ts`'s `main()` was
  > extracted to an exported `applyHttpServerTimeouts(httpServer, config)` in
  > `src/transport/http.ts` (`main()` itself isn't unit-testable without
  > booting the whole server) — tested against a real `node:http` `Server`
  > in `http.test.ts`; the Zod half lives in `config/index.test.ts`.

## 7. Docs and validation

- [x] 7.1 `README.md`: document the three new `transport.http` config keys, the
  ...
  image section.
- [x] 7.2 Mirror the same edits section-for-section in `README.de.md`,
  `README.es.md`, `README.fr.md`, `README.zh-CN.md`.
- [x] 7.3 Bump `package.json` `version` (user-facing: new config keys, changed
