...
- [x] 1.1 Add `MemoryOrigin` interface to `src/domain/types.ts` (near the
  `RecallFilter` interface at `src/domain/types.ts:23-26`):
  `{ session_id?: string; tool?: string; repo?: string; branch?: string }`.
- [x] 1.2 Add `origin: MemoryOrigin | null` and `confidence: number` to `MemoryRecord`
...
- [x] 1.3 Add `origin?: MemoryOrigin | null` and `confidence?: number` to `SearchResult`
...
- [x] 1.4 Extend `RememberInputSchema` in `src/domain/schemas.ts:26-36`:
  ```ts
  origin: z.object({
...
- [x] 2.1 Add `pipeline.default_confidence` to the Zod config schema
...
- [x] 2.2 No new env vars (mirrors the `search.ranking` precedent at
...
- [x] 3.1 Add `origin` (object, `additionalProperties: false`, matching 1.4's shape)
  ...
  line 15).
- [x] 3.2 In `handleRemember` (`src/tools/index.ts:133-154`), pass `origin: input.origin
  ?? undefined` and `confidence: input.confidence` through to `ctx.pipeline.process`.

## 4. Write pipeline

- [x] 4.1 Extend `WritePipeline.process`'s input type (`src/pipeline/index.ts:29-41`)
  and `decide`'s `input` parameter type (`src/pipeline/index.ts:82-93`) with
  `origin?: MemoryOrigin` and `confidence?: number`.
- [x] 4.2 In `decide` (`src/pipeline/index.ts:94-103`), resolve the effective
...
- [x] 4.3 UPDATE path (`src/pipeline/index.ts:162-195`): merge
...
- [x] 4.4 DELETE-then-ADD path (`src/pipeline/index.ts:197-248`): set
  `origin: input.origin ?? null, confidence,` on the `mem` object (lines 210-235).
- [x] 4.5 ADD path (`src/pipeline/index.ts:250-` end of `decide`): set
  `origin: input.origin ?? null, confidence,` on the `mem` object (lines 252-270+).

## 5. SQLite storage

- [x] 5.1 Add `origin TEXT,` and `confidence REAL NOT NULL DEFAULT 1.0,` columns to
  the `memories` table DDL (`src/storage/sqlite.ts`, after `embedding_model TEXT,` at
  line 197).
- [x] 5.2 Add both to the additive-migration `requiredColumns` array
  ...
  DEFAULT 1.0' }`.
- [x] 5.3 `insertMemory` (`src/storage/sqlite.ts:452-502`): compute `const origin =
...
- [x] 5.4 `upsertMemoryFromPayload` (`src/storage/sqlite.ts:504-539+`, the
...
- [x] 5.5 `rowToMemory` (`src/storage/sqlite.ts:1659-1687`): add
...
- [x] 6.1 Extend `toQdrantPayload`'s parameter type (`src/storage/index.ts:801-806`)
...
- [x] 6.2 Add `origin: mem.origin ?? null, confidence: mem.confidence,` to the
...
- [x] 7.1 `buildSearchResults`'s main loop (`src/search/index.ts:359-377`): add
  `origin: mem.origin, confidence: mem.confidence,` to the constructed `SearchResult`.
- [x] 7.2 `buildResultFromQdrantPayload` (`src/search/index.ts:399-420+`, the
...
- [x] 7.3 Confirm (no code change expected) that `memory://{id}` and `memory://list`
...
- [x] 8.1 `src/domain/schemas.test.ts`: `RememberInputSchema` accepts valid
  `origin`/`confidence`, rejects out-of-range `confidence` and unknown `origin` keys
  (`.strict()`), and defaults both to absent when omitted.
- [x] 8.2 `src/config/index.test.ts`: `pipeline.default_confidence` defaults and
  validates per-source bounds.
- [x] 8.3 `src/pipeline/index.test.ts`: ADD stamps the caller's `origin`/`confidence`;
...
- [x] 8.4 `src/storage/sqlite.test.ts`: `insertMemory`/`rowToMemory` round-trip
...
- [x] 8.5 `src/storage/index.test.ts` and/or `src/storage/qdrant.test.ts`:
  `toQdrantPayload` includes `origin`/`confidence` in the upserted payload.
- [x] 8.6 `src/search/index.test.ts`: `recall`/`search` results carry `origin`/
  `confidence` from the hydrated memory; the Qdrant-payload fallback path narrows both
  safely.
- [x] 8.7 `src/tools/index.test.ts`: `remember` accepts `origin`/`confidence` end to
  end; a call omitting them still succeeds unchanged (backward-compatibility
  regression).

## 9. Docs and validation

- [x] 9.1 Document `origin`/`confidence` in `README.md`'s `remember` input table
...
- [x] 9.2 Bump `package.json` `version` (currently `1.11.0`).
- [x] 9.3 Run `npm run lint` (tsc --noEmit + eslint) and `npm test`; fix any fallout.
