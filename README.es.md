...
| `include_archived` | `boolean` | No | `false` | También busca en memorias archivadas (ver [Decaimiento, Limpieza y Archivado](#decaimiento-limpieza-y-archivado)) mediante coincidencia de términos en el resumen/etiquetas: cada término de la consulta separado por espacios debe coincidir con el resumen conservado o las etiquetas (subcadena sin distinción de mayúsculas por término); una consulta sin términos no devuelve nada. Las coincidencias se añaden después de los resultados activos, marcadas con `archived: true`, y nunca reducen cuántos resultados activos permite `limit`. Las coincidencias archivadas no se registran como acceso. |
