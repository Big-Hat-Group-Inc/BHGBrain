...
MCP-Clients haben einen entsprechenden Pfad: Der Parameter `include_archived` des `search`-Tools findet archivierte Erinnerungen per Zusammenfassungs-/Tag-Begriffsabgleich — jeder durch Leerzeichen getrennte Suchbegriff muss unabhängig auf die gespeicherte Zusammenfassung oder die Tags passen (markiert mit `archived: true`, nie als Zugriff protokolliert) — und die `restore`-Aktion des `review`-Tools erstellt eine aktive Erinnerung aus einem Archiveintrag neu — markiert mit `restored-from-archive`, wobei die Archivzeile (anders als beim CLI-Pfad) **beibehalten** wird, sodass ihr Ursprung nachvollziehbar bleibt. Siehe [MCP-Tools-Referenz](#mcp-tools-referenz).
...
| `include_archived` | `boolean` | Nein | `false` | Durchsucht zusätzlich archivierte Erinnerungen (siehe [Verfall, Bereinigung und Archivierung](#verfall-bereinigung-und-archivierung)) per Zusammenfassungs-/Tag-Begriffsabgleich: Jeder durch Leerzeichen getrennte Suchbegriff muss auf die gespeicherte Zusammenfassung oder die Tags passen (Groß-/Kleinschreibung-unabhängiger Teilstring pro Begriff); eine Suchanfrage ohne Begriffe liefert keine Treffer. Treffer werden nach den aktiven Ergebnissen angehängt, mit `archived: true` markiert und reduzieren nie, wie viele aktive Ergebnisse `limit` zulässt. Archivtreffer werden nicht als Zugriff protokolliert. |
